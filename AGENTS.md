...
- `@opencode-ai/plugin` must be in **dependencies** (not `devDependencies`) in `package.json`. When OpenCode installs the plugin from git into `~/.cache/opencode/packages/doodba-dev@git+.../`, npm only installs `dependencies`. The plugin's `.opencode/plugins/doodba-dev.js` imports TypeScript files (`src/tools/index.ts`) at runtime using Bun's transpiler; those TS files import `@opencode-ai/plugin` for type definitions. Without it in the cache's `node_modules`, Bun fails with "Cannot find module '@opencode-ai/plugin'". The redundancy in the dev workspace is acceptable; a broken published plugin is not. **Test coverage gap:** Local tests always have access to `devDependencies`, so they never catch this. An integration test running `npm pack && npm install --omit=dev` on the extracted tarball would have caught it.
